...
//                }
    ...
    public void displayStatus(final ParcelableStatus status) {
        ...
        final boolean nicknameOnly = mPreferences.getBoolean(KEY_NICKNAME_ONLY, false);
        ...
        mNameView.setText(TextUtils.isEmpty(nick) ? status.user_name : nicknameOnly ? nick : getString(
        ...
        final String in_reply_to = getDisplayName(getActivity(), status.in_reply_to_user_id, status.in_reply_to_name,
                status.in_reply_to_screen_name, nameFirst, nicknameOnly, true);
