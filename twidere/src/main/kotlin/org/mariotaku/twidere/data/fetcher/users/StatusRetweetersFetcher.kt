...
class StatusRetweetersFetcher(private val statusId: String) : UsersFetcher {
    ...
    override fun forTwitter(account: AccountDetails, twitter: Twitter, paging: Paging): List<User> {
        return if (account.official) {
            twitter.getRetweetedBy(statusId, paging)
        } else {
            twitter.lookupUsers(twitter.getRetweetersIDs(statusId, paging).iDs)
        }
    }
