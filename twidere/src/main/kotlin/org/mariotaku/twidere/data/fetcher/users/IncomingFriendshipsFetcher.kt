...
class IncomingFriendshipsFetcher : UsersFetcher {
    override fun forFanfou(account: AccountDetails, fanfou: Fanfou, paging: Paging): List<User> {
        return fanfou.getFriendshipsRequests(paging)
    }

    override fun forMastodon(account: AccountDetails, mastodon: Mastodon, paging: Paging): LinkHeaderList<Account> {
        return mastodon.getFollowRequests(paging)
    }

    override fun forTwitter(account: AccountDetails, twitter: Twitter, paging: Paging): List<User> {
        return twitter.lookupUsers(twitter.getIncomingFriendships(paging).iDs)
    }

    override fun forStatusNet(account: AccountDetails, statusNet: StatusNet, paging: Paging): List<User> {
        return statusNet.lookupUsers(statusNet.getIncomingFriendships(paging).iDs)
    }
