...
class AuthenticatedUriLoader(
    ...
    override fun handles(model: AuthenticatedUri): Boolean = true

    override fun buildLoadData(model: AuthenticatedUri, width: Int, height: Int, options: Options): ModelLoader.LoadData<InputStream>? {
    ...
    class Factory(val context: Context, val client: OkHttpClient) : ModelLoaderFactory<AuthenticatedUri, InputStream> {
        override fun build(multiFactory: MultiModelLoaderFactory) = AuthenticatedUriLoader(context, client)
