...
public final class InternalParseUtil {
    ...
    @MicroBlogResponse.AccessLevel
    public static int toAccessLevel(final HttpResponse res) {
        if (res == null) return MicroBlogResponse.AccessLevel.NONE;
        final String xAccessLevel = res.getHeader("X-Access-Level");
        int accessLevel;
        if (null == xAccessLevel) {
            accessLevel = MicroBlogResponse.AccessLevel.NONE;
        } else {
            ...
            switch (xAccessLevel.length()) {
                // “read” (Read-only)
                case 4:
                    accessLevel = MicroBlogResponse.AccessLevel.READ;
                    break;
                case 10:
                    // “read-write” (Read & Write)
                    accessLevel = MicroBlogResponse.AccessLevel.READ_WRITE;
                    break;
                case 25:
                    // “read-write-directmessages” (Read, Write, & Direct
                    // Message)
                    accessLevel = MicroBlogResponse.AccessLevel.READ_WRITE_DIRECTMESSAGES;
                    break;
                case 26:
